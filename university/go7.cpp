...
// UCBが一番高い手を選ぶ
int select_best_ucb(int node_n, const double C, int algo)
{
	NODE *pN = &node[node_n];
	int select = -1;
	double max_ucb = -999;
...
int search_uct(int color, int node_n, const double C, int algo)
{
	NODE *pN = &node[node_n];
	CHILD *c = NULL;
	...
	for (;;)
	{
		int select = select_best_ucb(node_n, C, algo);
		c = &pN->child[select];
		int z = c->z;
		int err = move(z, color, FILL_EYE_ERR);
		if (err == 0)
	...
	else
	{
		if (c->next == NODE_EMPTY)
			c->next = create_node();
		win = -search_uct(flip_color(color), c->next, C, algo);
	}
...
int select_best_uct(int color, const double C, int algo)
{
	node_num = 0;
...
int main(int argc, char *argv[])
{
	...
	{
		double C_black = 0.1 * _i;
		for (int _j = 0; _j < 10; _j++)
		{
			double C_white = _j * 0.1;
...
//		else             z = select_best_move(color);
		...
		int z = select_best_move(color);	// 原始モンテカルロ
					...
					if (color == WHITE)
					{
						z = select_best_uct(color, C_white); // UCT
					}
					else
					{
						z = select_best_uct(color, C_black); // UCT
					}
					*/
					int z = select_best_uct(color, C_black, UCB1);
#endif
