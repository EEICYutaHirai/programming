...
int select_best_ucb(int node_n, int color)
{
    ...
    for (i = 0; i < pN->child_num; i++)
    {
        ...
        else
        {
            ...
            if (color == 1)
            {
                ucb = C * sqrt(log(pN->child_games_sum) / c->games) + bakuchi_black * c->bakuchi_rate;
            }
            else
            {
                ucb = c->rate + C * sqrt(log(pN->child_games_sum) / c->games) + bakuchi_white * c->bakuchi_rate;
            }
        }
        if (ucb > max_ucb)
        {
