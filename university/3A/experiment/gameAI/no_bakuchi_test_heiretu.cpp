...
double no_bakuchi_black;
double no_bakuchi_white;
int color;
double bakuchi_param_black;
double bakuchi_param_white;
...
// 地を数えて勝ちか負けかを返す
double count_score(int turn_color)
{
    double score = 0;
    ...
    for (y = 0; y < B_SIZE; y++)
        for (x = 0; x < B_SIZE; x++)
        {
            ...
            for (i = 0; i < try_num; i++)
            {
                ...
                int win = playout(flip_color(color)) > 0 ? -1 : 1;
                win_sum += win;
                //			print_board();
                //			printf("win=%d,%d\n",win,win_sum);
    ...
    else
    {
        for (i = 0; i < pN->child_num; i++)
        {
            ...
            else
            {
                ...
                else if (algo == NO_BAKUCHI)
                {
                    const double C = 0.31;
                    if (color == BLACK)
                    {
                        ucb = c->rate + C * sqrt(log(pN->child_games_sum) / c->games) + no_bakuchi_black * c->bakuchi_rate;
                    }
                    else
                    {
                        ucb = c->rate + C * sqrt(log(pN->child_games_sum) / c->games) + no_bakuchi_white * c->bakuchi_rate;
                    }
                }
            }
            if (ucb > max_ucb)
...
double search_uct(int color, int node_n, int algo)
{
    NODE *pN = &node[node_n];
    CHILD *c = NULL;
    ...
    { // 最初の1回目はplayout. <= 10 でノード数を減らせる
        how_win = -playout(flip_color(color));
    }
    else
    {
        if (c->next == NODE_EMPTY)
            c->next = create_node();
        how_win = -search_uct(flip_color(color), c->next, algo);
    }
    win = how_win > 0 ? -1 : 1;
    how_win = how_win > 0 ? (1. / how_win) : 0.;
    // 勝率を更新
    c->rate = (c->rate * c->games + win) / (c->games + 1);
    c->bakuchi_rate = (c->bakuchi_rate * c->games + how_win) / (c->games + 1);
    c->games++;            // この手の回数を更新
...
int main(int argc, char *argv[])
{
    ...
    int black_algo = NO_BAKUCHI;
    int white_algo = UCB1;
    //int black_rate = atoi(argv[3]);
    ...
    for (int _i = 0; _i < 10; _i++)
    {
        ...
        {
            no_bakuchi_black = _i * .5;
            //no_bakuchi_white = _j * .5;
...
//		else             z = select_best_move(color);
		...
		int z = select_best_move(color);	// 原始モンテカルロ
            ...
            cout << no_bakuchi_black << " " << no_bakuchi_white << endl;
        }
    }
