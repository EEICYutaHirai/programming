...
int ko_z;
int all_playouts = 0;
double bakuchi_black;
double bakuchi_white;
...
int select_best_ucb(int node_n, int color)
{
    ...
    for (i = 0; i < pN->child_num; i++)
    {
        ...
        else
        {
            ...
            if (color == 1)
            {
                ucb = c->rate + C * sqrt(log(pN->child_games_sum) / c->games) + bakuchi_black * c->bakuchi_rate;
            }
            else
            {
                ucb = C * sqrt(log(pN->child_games_sum) / c->games) + bakuchi_white * c->bakuchi_rate;
            }
        }
        if (ucb > max_ucb)
        {
...
int search_uct(int color, int node_n)
{
    ...
    for (;;)
    {
        int select = select_best_ucb(node_n, color);
        c = &pN->child[select];
        int z = c->z;
        int err = move(z, color, FILL_EYE_ERR);
        if (err == 0)
    ...
    else
    {
        if (c->next == NODE_EMPTY)
            c->next = create_node();
        how_win = -search_uct(flip_color(color), c->next);
    }
    win = how_win > 0 ? 1 : -1;
    double bakuchi_delta = how_win > 0 ? ((100. - how_win) / 100.) : 0;

    // 勝率を更新
    c->rate = (c->rate * c->games + win) / (c->games + 1);
    c->bakuchi_rate = (c->bakuchi_rate * c->games + bakuchi_delta) / (c->games + 1);
    c->games++;            // この手の回数を更新
    pN->child_games_sum++; // 合計回数も更新
    return win;
}

int uct_loop = 1000; // uctでplayoutを行う回数
...
int main()
{
    ...
    for (int p = 1; p < 10; p++)
    {
        ...
        cout << bakuchi_black << "   " << bakuchi_white << endl;
        cout << n_win << endl;
    }
