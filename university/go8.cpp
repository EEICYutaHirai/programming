...
int yokubari_rate_black;
int yokubari_rate_white;
int color;
...
int select_best_ucb(int node_n, int algo)
{
	...
	if (algo == YOKUBARi)
	{
		if (color = WHITE && (rand() % 100) / 100. <= yokubari_rate_white)
		{
			select = rand() % (pN->child_num);
		}
		if (color = BLACK && (rand() % 100) / 100. <= yokubari_rate_black)
		{
			select = rand() % (pN->child_num);
		}
	}
...
//		else             z = select_best_move(color);
	...
	cout << "n_black_win" << n_black_win << endl;
